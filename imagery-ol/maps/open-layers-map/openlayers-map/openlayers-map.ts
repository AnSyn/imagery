...
export class OpenLayersMap extends BaseImageryMap<OLMap> {
	...
	static groupLayers = new Map<StaticGroupsKeys, Group>(
		Object.values(StaticGroupsKeys).map((key) => [key, new Group()]) as any
	);
	...
	constructor(
		...
		this.olConfig.tilesLoadingDoubleBuffer = this.olConfig
			.tilesLoadingDoubleBuffer || {
			debounceTimeInMs: 500,
			timeoutInMs: 3000,
		};
		this.mapObject = this._mapObject;
	}
	...
	signalWhenTilesLoadingEnds() {
		...
		this.tilesLoadProgressEventEmitter
			.pipe(
				...
				debounceTime(
					this.olConfig.tilesLoadingDoubleBuffer.debounceTimeInMs
				), // Adding debounce, to compensate for strange multiple loads when reading tiles from the browser cache (e.g. after browser refresh)
				takeUntil(
					timer(
						this.olConfig.tilesLoadingDoubleBuffer.timeoutInMs
					).pipe(
						tap(() => {
	...
	initListeners() {
		...
		this.subscribers.push(
			this.getMoveEndPositionObservable
				.pipe(
					switchMap((a) => {
			...
			this.getMoveStartPositionObservable
				.pipe(
					switchMap((a) => {
						return this.getPosition();
					})
	...
	public resetView(
		...
		useDoubleBuffer =
			useDoubleBuffer && !layer.get(ImageryLayerProperties.FROM_CACHE);
		if (useDoubleBuffer) {
		...
		const rotation: number =
			this._mapObject.getView() && this.mapObject.getView().getRotation();
		...
		if (useDoubleBuffer) {
			...
			return this._setMapPositionOrExtent(
				this.backgroundMapObject,
				position,
				extent,
				rotation
			).pipe(
	...
	public getLayerById(id: string): ol_Layer {
		return <ol_Layer>this.mapObject
			.getLayers()
			.getArray()
			.find((item) => item.get(ImageryLayerProperties.ID) === id);
	}
	...
	getMainLayer(): ol_Layer {
		const mainLayer = this._mapLayers.find(
			(layer: ol_Layer) =>
				layer.get(ImageryLayerProperties.NAME) ===
				IMAGERY_MAIN_LAYER_NAME
		);
		return mainLayer;
	}
	...
	getLayerByName(name: string): ol_Layer {
		const layer = this._mapLayers.find(
			(layer: ol_Layer) => layer.get(ImageryLayerProperties.NAME) === name
		);
		return layer;
	}
	...
	fitToExtent(
		extent: ImageryMapExtent,
		map: OLMap = this.mapObject,
		view: View = map.getView()
	) {
		const collection: any = turf.featureCollection([
			ExtentCalculator.extentToPolygon(extent),
		]);

		return this.projectionService
			.projectCollectionAccuratelyToImage<olFeature>(collection, map)
			.pipe(
				tap((features: olFeature[]) => {
					view.fit(features[0].getGeometry() as olPolygon, {
						nearest: true,
	...
	public addMapLayer(layer: ol_Layer) {
		...
		const baseMapLayer = this._mapLayers.find(
			(layer: ol_Layer) =>
				layer.get(ImageryLayerProperties.NAME) ===
				IMAGERY_BASE_MAP_LAYER
		);
		...
		if (
			layer.get(ImageryLayerProperties.ID) !==
			main.get(ImageryLayerProperties.ID)
		) {
			this.addLayer(layer);
		}
	...
	public removeAllLayers() {
		this.showGroups.forEach((show, group) => {
			if (show && this._mapObject) {
				this._mapObject.removeLayer(
					OpenLayersMap.groupLayers.get(group)
				);
			}
		});
	...
	public removeLayer(layer: ol_Layer): void {
		...
		this._mapLayers = this._mapLayers.filter(
	...
	public setCenter(
		...
		return this.projectionService
			.projectAccuratelyToImage(center, this.mapObject)
			.pipe(
				map((projectedCenter) => {
					const olCenter = <[number, number]>(
						projectedCenter.coordinates
					);
					if (animation) {
						this.flyTo(olCenter);
					} else {
	...
	calculateRotateExtent(olmap: OLMap): Observable<{
		extentPolygon: ImageryMapExtentPolygon;
		layerExtentPolygon: ImageryMapExtentPolygon;
	}> {
		...
		const cachedMainExtent = mainLayer.get(
			ImageryLayerProperties.MAIN_EXTENT
		);
		...
		if (mainExtent && !Boolean(cachedMainExtent)) {
			...
			return this.projectionService
				.projectCollectionAccurately(
					[
						new olFeature(new olPolygon(coordinates)),
						new olFeature(layerExtentPolygon),
					],
					olmap
				)
				.pipe(
					map((collection: FeatureCollection<GeometryObject>) => {
						mainLayer.set(
							ImageryLayerProperties.MAIN_EXTENT,
							collection.features[1].geometry as Polygon
						);
						return {
							extentPolygon: collection.features[0]
								.geometry as Polygon,
							layerExtentPolygon: collection.features[1]
								.geometry as Polygon,
						};
					})
				);
		...
		return this.projectionService
			...
			.pipe(
				map((features: olFeature[]) => {
					...
					const rotation =
						ExtentCalculator.calcRotation(geoJsonExtent);
					const resolution = ExtentCalculator.calcResolution(
						geoJsonExtent,
						olmap.getSize(),
						rotation
					);
	...
	public setPosition(
		...
		const someIsNan = !extentPolygon.coordinates[0].every(
			areCoordinatesNumeric
		);
		if (someIsNan) {
			console.warn(
				"ol map setposition failed, can't handle invalid coordinates " +
					extentPolygon
			);
			return of(true);
		}
		...
		const isProjectedPosition =
			projectedState &&
			viewProjection.getCode() === projectedState.projection.code;
	...
	public getPosition(): Observable<IImageryMapPosition> {
		...
		return this.calculateRotateExtent(this.mapObject).pipe(
			map(
				({
					extentPolygon: extentPolygon,
					layerExtentPolygon: layerExtentPolygon,
				}) => {
					if (!extentPolygon) {
						return null;
					}
					...
					if (someIsNaN) {
						console.warn(
							"ol map getPosition failed invalid coordinates ",
							extentPolygon
						);
						return null;
					}
					...
					if (
						this.olConfig.needToUseLayerExtent &&
						this.needToUseLayerExtent(
							layerExtentPolygon,
							extentPolygon
						)
					) {
	...
	needToUseLayerExtent(
		...
		let cornersInside = 0;
		for (let i = 0; i < extentPolygon.coordinates[0].length - 1; i++) {
			// -1 in order to ignore duplicated coordinate
			const isInside = turf.booleanPointInPolygon(
				turf.point(extentPolygon.coordinates[0][i]),
				turf.polygon(layerExtentPolygon.coordinates),
	...
	getCoordinateFromScreenPixel(screenPixel: {
		...
		const coordinate = this.mapObject.getCoordinateFromPixel([
			screenPixel.x,
			screenPixel.y,
		]);
		return coordinate;
	...
	private _pointerMoveListener: (args) => void = (args) => {
		...
		return this.projectionService
			.projectApproximately(point, this.mapObject)
			.pipe(
				take(1),
				tap((projectedPoint) => {
	...
	private _rightClickListener: (args) => void = (event: MouseEvent) => {
		let coordinates = this.getCoordinateFromScreenPixel({
			x: event.offsetX,
			y: event.offsetY,
		});
	...
	rasiseEvent(
		event: any,
		eventEmitter: EventEmitter<IMouseClick>,
		coordinates?: any
	) {
		const point = <GeoPoint>(
			turf.geometry("Point", event.coordinate || coordinates)
		);
		return this.projectionService
			.projectAccurately(point, this.mapObject)
			.pipe(
				take(1),
				tap((projectedPoint) => {
					if (areCoordinatesNumeric(projectedPoint.coordinates)) {
	...
	// Used by resetView()
	private _setMapPositionOrExtent(
		map: OLMap,
		position: IImageryMapPosition,
		extent: ImageryMapExtent,
		rotation: number
